...
### Mise à niveau des dépendances Expo (SDK 55)

Si Expo affiche un message "expected version" pour plusieurs paquets, alignez les versions avec:

```bash
npx expo install expo@~55.0.0 expo-constants expo-router expo-status-bar react react-dom react-native react-native-safe-area-context react-native-screens react-native-web
npm install -D @types/react@~19.2.10 typescript@~5.9.2
```
